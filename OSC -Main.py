...
# HTML is fetched live from GitHub on every request
_HTML_CACHE = None
_HTML_CACHE_TIME = 0
_HTML_URL = "https://raw.githubusercontent.com/bingnut/Quest-OSC-engine/main/OSC%20HTML%20SERVER.html"

def get_html_page():
    """Fetch HTML from GitHub repo, cache for 60 seconds."""
    global _HTML_CACHE, _HTML_CACHE_TIME
    import time as _time
    now = _time.time()
    if _HTML_CACHE and (now - _HTML_CACHE_TIME) < 60:
        return _HTML_CACHE
    try:
        req = urllib.request.Request(_HTML_URL, headers={
            "User-Agent": "OSC-Quest-Engine/1.0",
        })
        with urllib.request.urlopen(req, timeout=8) as resp:
            html_bytes = resp.read()
        _HTML_CACHE = html_bytes
        _HTML_CACHE_TIME = now
        print(f'[OSC Quest Engine] HTML loaded from GitHub ({len(html_bytes)} bytes)')
        return html_bytes
    except Exception as exc:
        print(f'[OSC Quest Engine] GitHub fetch failed: {exc}')
        if _HTML_CACHE:
            print('[OSC Quest Engine] Serving cached version')
            return _HTML_CACHE
        fallback = (
            b'<!DOCTYPE html><html><head><meta charset=UTF-8>'
            b'<title>OSC Quest Engine</title>'
            b'<style>body{background:#0d0d0f;color:#e8e8f0;font-family:sans-serif;'
            b'display:flex;align-items:center;justify-content:center;'
            b'height:100vh;margin:0;flex-direction:column;gap:12px}'
            b'h1{color:#7c6aff}p{color:#6b6b80;font-size:13px}a{color:#5eead4}</style></head>'
            b'<body><h1>&#9881; OSC Quest Engine</h1>'
            b'<p>Could not load dashboard from GitHub.</p>'
            b'<p>Check your internet and <a href="/">refresh</a>.</p>'
            b'</body></html>'
        )
        return fallback
